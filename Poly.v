...
Theorem app_assoc : forall A (l m n:list A),
  l ++ m ++ n = (l ++ m) ++ n.
Proof.
  intros A l m n. induction l as [| n0 l' IHl'].
  - reflexivity.
  - simpl. rewrite IHl'. reflexivity. Qed.
...
Fixpoint split {X Y : Type} (l : list (X*Y))
  ...
  match l with
  | [] => ([], [])
  | h :: t => (((cons (fst h) (fst (split t)))),
                (cons (snd h) (snd (split t))))
  end.
...
(** **** Exercise: 3 stars, standard (map_rev)  
    ...
    auxiliary lemma. *)
Search rev.
Lemma map_dist : forall (X Y : Type) (f : X -> Y) (l1 : list X) (l2 : list X),
...
Proof.
  ...
  - reflexivity.
    - simpl. rewrite map_dist. rewrite IHl'. reflexivity. Qed.
(** [] *)
...
Example fold_pairs :
  fold (fun n l => cons (n, n) l) [1; 2] [] = [(1,1);(2,2)].
Proof. reflexivity. Qed.
...
Lemma pair_fst_snd : forall (X Y : Type) (p : X * Y),
    ((fst p), (snd p)) = p.
Proof.
  intros X Y p. unfold fst. unfold snd. **)
...
*)
(* since n is the length of l, it will never hit the n =? 0 case
so it will have to reach [] => None case eventually. *)
...
Definition exp (n m : cnat) : cnat :=
  fun (X : Type) (f : X -> X) (x : X) => one X (m (X -> X) (n X) f) x.
